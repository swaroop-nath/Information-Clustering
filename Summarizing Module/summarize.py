...
def summarize_docs(docs: List[str]) -> str:
    ...
    start = time()
    mgr.logging.info('method: summarize_docs- Finding abbreviations in the text')
    print('method: summarize_docs- Finding abbreviations in the text')
    abbrevs = find_abbreviations(docs)
    end = time()
    print('Time taken to find abbreviations: {:03f} seconds'.format(end - start))

    start = time()
    mgr.logging.info('method: summarize_docs- Extracting top_k paragraphs from the document')
    print('method: summarize_docs- Extracting top_k paragraphs from the document')
    ...
    start = time()
    mgr.logging.info('method: summarize_docs- Post Processing the output of gathering important paragraphs')
    print('method: summarize_docs- Post Processing the output of gathering important paragraphs')
    ...
    start = time()
    mgr.logging.info('method: summarize_docs- Starting clustering of sentences in important paragraphs')
    print('method: summarize_docs- Starting clustering of sentences in important paragraphs')
    clusters, noisy_data = extract_clusters(final_doc)
    end = time()
    print('Time taken to cluster sentences: {:03f} seconds'.format(end - start))

    start = time()
    mgr.logging.info('method: summarize_docs- Extracting a good path for each of the found cluster')
    print('method: summarize_docs- Extracting a good path for each of the found cluster')
    cluster_paths = []
    ...
    start = time()
    mgr.logging.info('method: summarize_docs- Ranking sentences for preparing final summary')
    print('method: summarize_docs- Ranking sentences for preparing final summary')
